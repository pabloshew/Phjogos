class Bola{
    constructor(c, pad){
        ...
        this.dirY=0;
        this.movendo= false;
        this.x=this.y=(this.c.canvas.width/2)-(this.largura/2);
        this.y=(this.c.canvas.height/2)-(this.altura/2);
        this.bolao= new Image;
        this.bolao.src = "Assets/Poke.png";
        this.bolao.width= 50;
        this.bolao.height= 50;
        this.bolao.addEventListener('load', () => {
            this.draw()
        });
        ...
        if(this.movendo){
            ...
            if(this.y >=(this.c.canvas.height - this.altura)){
                ...
                if((this.x <= this.pad.x + this.pad.largura) && (this.x + this.largura >= this.pad.x)
                    ...
                    if((this.x <= this.pad.x + this.pad.largura) && (this.x + this.largura >= this.pad.y) && ((this.y + this.altura >= this.pad.y) && (this.y <= this.pad.altura+this.pad.y)))
                        {
                            this.dirX = -1;
...
resetBall(){
    ...
    draw(){
        ...
        this.c.drawImage(this.bolao, this.x, this.y, this.bolao.width, this.bolao.height);
